...
-- 1/ mise en ordre des tables.

CREATE DATABASE 'cas_gescom';                                                         -- Créer la base de donnée Cas_gescom
DROP TABLE IF EXISTS `countries`;                                                     -- Créer la table si elle n'existe pas
CREATE TABLE IF NOT EXISTS `countries` (
  `cou_id` char(2) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,         -- CHARACTER SET utf8 est un codage de caractères informatiques conçu pour coder l’ensemble des caractères du 
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8;                                           -- InnoDB est un moteur de stockage pour les systèmes de gestion de base de données MySQL et MariaDB.
                                                                                -- celui-ci est utilisé pour la gestion des clés étrangères.


DROP TABLE IF EXISTS `posts`;
CREATE TABLE IF NOT EXISTS `posts` (
  `pos_id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `pos_libel` varchar(30) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  PRIMARY KEY (`pos_id`)
...
-------------------------------------------
-- Données concernant toutes les tables
...
INSERT INTO `posts` (`pos_id`, `pos_lib`) VALUES
(1, 'Comptabilité'),
...
-- Dans un 1er temps on remplira d'abord les tables Categories, Posts et Countries, car elle n'ont pas de clé étrangères.
-- ensuite on rempli Employees, car elle a besoin de Posts qui est déjà préremplie.
-- De plus on peut aussi remplir Suppliers et Customers, qui ont besoin de Countries qui sont déjà remplies.
-- Puis On rempli Products, qui a besoin de Categories et Suppliers.
-- Et on rempli Orders, qui reprend les ID de Customers.
-- pour conclure Orders_details, qui a besoin d'un ID d'Orders et de Products.
